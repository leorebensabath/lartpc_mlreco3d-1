...
class ClusteringLoss(torch.nn.Module):
    ...
    def intra_cluster_loss(self, features, seediness, labels, cmeans):
        ...
        # This version uses the Laplacian Kernel
        with torch.no_grad():
        ...
        for i, c in enumerate(cluster_labels):
            ...
            margin = - var * np.log(self._threshold)
            # Hinge Loss
            loss = torch.mean(torch.pow(torch.clamp(dist1 - margin, min=0), 2))
            if dist2.nelement() > 0:
                loss += torch.mean(torch.pow(torch.clamp(margin - dist2, min=0), 2))
            var_loss += loss
            # Seediness Truth
            seed_truth[labels == c] = torch.exp(-dist1 / var).detach()

        var_loss /= float(len(cmeans))
        smoothness_loss /= float(len(cmeans))

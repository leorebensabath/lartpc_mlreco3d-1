...
class UResNet(torch.nn.Module):
    ...
    def __init__(self, cfg, name="uresnet_clustering"):
        ...
        # Behavior of torch.nn.Linear different from Submanifold 1x1 Conv
        if self._coordConv:
            self.emb_linear = torch.nn.Linear(m+3, self._hypDim)
        else:
    ...
    def forward(self, input):
        ...
        for i, layer in enumerate(self.decoding_conv):
            if self._N > 0:
                ...
                coords = x_emb.get_spatial_locations().numpy()
                perm = np.lexsort((coords[:, 2], coords[:, 1], coords[:, 0], coords[:, 3]))
                embedding.append(x_emb.features[perm])
                ...
                coords = x.get_spatial_locations().numpy()
                perm = np.lexsort((coords[:, 2], coords[:, 1], coords[:, 0], coords[:, 3]))
                embedding.append(x.features[perm])

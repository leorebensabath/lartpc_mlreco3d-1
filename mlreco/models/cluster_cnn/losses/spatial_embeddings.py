...
# class MultiLayerLovasz(MaskLovaszInterLoss):
#
#     def __init__(self, cfg, name='clustering_loss'):
#         super(MultiLayerLovasz, self).__init__(cfg)
#         self.l1loss = torch.nn.L1Loss()
#         self.cosine_embedding_loss = torch.nn.CosineEmbeddingLoss(margin=0.1)
#
#     def layer_similarity_loss(self, embeddings, clabels):
#         '''
#
#         '''
#         loss = 0.0
...
#             mask[index] = 1
#             mask[~index] = -1
#             loss += self.cosine_embedding_loss(centroid[i], embeddings, mask)
#
#         loss /= n_clusters
#         return loss
#
#     def combine_multiclass_2(self, embeddings, slabels, clabels):
#         loss = 0
#         semantic_classes = slabels.unique()
#         for sc in semantic_classes:
#             index = (slabels == sc)
#             loss += self.layer_similarity_loss(embeddings[index], clabels[index])
#         loss /= len(semantic_classes)
#
#         return loss
#
#     def compute_loss_layer_2(self, embeddings, slabels, clabels):
#         loss = defaultdict(list)
#         accuracy = defaultdict(list)
#         batch_idx = slabels[:, 3].unique().int()
...
#             l = self.combine_multiclass_2(embedding_batch)
#             loss.append(l)
#
...
#         '''
#         MultiLayer Gaussian Mixture Loss
#         '''
#         loss = defaultdict(list)
#         accuracy = defaultdict(list)
#         num_gpus = len(semantic_labels)
#         num_layers = len(semantic_labels[0])
#
#         for i_gpu in range(num_gpus):
#             batch_idx = semantic_labels[i_gpu][0][:, 3]
#             batch_idx = batch_idx.unique()
#             batch_size = len(batch_idx)
#             # Summing clustering loss over layers.
#             for i, em in enumerate(out['features_cluster'][i_gpu][::-1]):
#                 slabel_layer = semantic_labels[i_gpu][i]
#                 perm = np.lexsort(slabel_layer[:, :4].detach().cpu().numpy().T)
#                 slabel_layer = slabel_layer[perm]
#                 clabel_layer = group_labels[i_gpu][i]
#                 perm = np.lexsort(clabel_layer[:, :4].detach().cpu().numpy().T)
#                 clabel_layer = clabel_layer[perm]
#                 print(slabel_layer)
#                 print(clabel_layer)
#                 coords = em.get_spatial_locations()
#                 print(coords)
#                 loss_layer = self.compute_loss_layer_2(em, slabel_layer, clabel_layer)
#                 loss['intermediate_embedding_loss'].append(loss_layer)
#
#             slabels = semantic_labels[i_gpu][0][:, -1]
#             clabels = group_labels[i_gpu][0][:, -2]
#             batch_idx = semantic_labels[i_gpu][0][:, 3]
#             embedding = out['embeddings'][i_gpu]
